...
const CurrencyExchange: React.FC<CurrencyExchangePropsType> = ({
    //старый подход деструктуризации пропсов
                                                                                  currenciesName,
                                                                                  currentCurrency,
                                                                                  currencyRate,
                                                                                  isBuying,
...
//data-* атрибуты позволяют хранить дополнительную инфу в стандартных элементах HTML, те использовать тег как некий стейт и потом jsкриптом эти данные забирать; любой атрибут чье имя начинается с data-* является  дата-атрибутом,
    return (
            ...
            <div className="currency-names">
                ...
                <ul>
                    {currenciesName.map((currency: string, index: number) => {
                        //массив currenciesName отвечает за отрисовку кномок с именем валюты
                        return (
                            <li
                                key={`${index}-${currency}`}
                                //проставлятеся класс active, чтоб было видно какая кнопка нажата на данный момент
                                className={`currencies ${currentCurrency === currency ? 'activeCurrency' : null}`}
                                //одна функция changeCurrentCurrency на три кнопки
                                onClick={changeCurrentCurrency}
                                //указан дата-атрибут: будет просто имя валюты для каждой кнопки свое
